"""
Dhan WS HA Paper/Live Trader  |  GUI Edition  v8
New: 3/5/7/9m TFs, Global SL%, MCX session end time, Save/Load Settings
"""
...
class TokenTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.log_box = ctk.CTkTextbox(self, height=160, font=F_MONO_S,
        ...
        self.shared_lbl = ctk.CTkLabel(sf, text="Checking…", width=220,
        ...
        self.after(500, self._check_shared_status)
        ...
        def _run():
            try:
                ...
                if verify_token(cfg["client_id"], cfg["access_token"]):
                    self.after(0, lambda: self._log("✅  Token is VALID."))
        ...
        try:
            ...
            if shared.get("access_token"):
                self.shared_lbl.configure(
                    text=f"✅  Token found: {shared['access_token'][:20]}…",
                    text_color="#3fb950")
            elif SHARED_TOKEN_FILE.exists():
                self.shared_lbl.configure(text="⚠️  File empty/invalid", text_color=ORANGE_COL)
            else:
                self.shared_lbl.configure(text=f"Not found", text_color=GREY_COL)
        except Exception as e:
    ...
    def _load_from_shared(self):
        try:
            ...
            if not shared.get("access_token"):
                self._log(f"❌  No token in shared file at {SHARED_TOKEN_FILE}")
                return
            ...
            self._log(f"✅  Loaded from dhan-token-generator: {token[:28]}…")
...
class StrategyTab(ctk.CTkFrame):
    def __init__(self, master):
        ...
        self._running      = False
        self._build()
        self._load_settings()   # auto-load saved settings on startup
    ...
    def _build(self):
        ...
        self.e_trig = ctk.CTkEntry(self.trig_frame, width=80, height=36,
                                    ...
                                    text_color=WHITE_COL, font=F_MONO_S)
        self.e_trig.insert(0, "2.0")
        self.e_trig.pack(side="left")
        ...
        ctk.CTkLabel(row4, text="MCX Session End (IST):", font=F_LABEL,
                     text_color=WHITE_COL).pack(side="left", padx=(4, 6))
        self.e_mcx_h = ctk.CTkEntry(row4, width=52, height=36,
                                     fg_color=PANEL_BG, border_color=BORDER,
                                     text_color=WHITE_COL, font=F_MONO_S)
        self.e_mcx_h.insert(0, "23")
        self.e_mcx_h.pack(side="left", padx=(0, 2))
        ctk.CTkLabel(row4, text=":", font=F_LABEL, text_color=GREY_COL).pack(side="left")
        self.e_mcx_m = ctk.CTkEntry(row4, width=52, height=36,
                                     fg_color=PANEL_BG, border_color=BORDER,
                                     text_color=WHITE_COL, font=F_MONO_S)
        self.e_mcx_m.insert(0, "30")
        self.e_mcx_m.pack(side="left", padx=(2, 4))
        ...
        ctk.CTkLabel(self.kc_frame, text="Keltner →",
                     font=("Segoe UI",11,"bold"), text_color=CYAN_COL
        ...
        ctk.CTkLabel(self.rsi_frame, text="RSI →",
                     font=("Segoe UI",11,"bold"), text_color=ORANGE_COL
                     ).pack(side="left", padx=(14,8))
        ...
        ctk.CTkButton(
            ...
            text_color=CYAN_COL, font=F_BTN,
    ...
    def _collect_settings(self) -> dict:
        """Collect all current GUI parameter values into a dict."""
        return {
            "tf":           self.tf_dd.get(),
            "variation":    self.var_dd.get(),
            "mode":         self._mode_var.get(),
            "order_type":   self.order_dd.get(),
            "trig_offset":  self.e_trig.get(),
            "lmt_offset":   self.e_lmt.get(),
            "global_sl":    self.e_gsl.get(),
            "mcx_h":        self.e_mcx_h.get(),
            "mcx_m":        self.e_mcx_m.get(),
            "kc_len":       self.e_kc_len.get(),
            "kc_atr":       self.e_kc_atr.get(),
            "kc_mult":      self.e_kc_mult.get(),
            "rsi_len":      self.e_rsi_len.get(),
            "rsi_buy":      self.e_rsi_buy.get(),
            "rsi_sell":     self.e_rsi_sell.get(),
            "symbols": {
                sym: {
                    "enabled": self._sym_vars[sym].get(),
    ...
    def _apply_settings(self, s: dict):
        """Apply a settings dict to all GUI widgets."""
        if s.get("tf") and s["tf"] in TF_OPTIONS:
            self.tf_dd.set(s["tf"])
        if s.get("variation") and s["variation"] in [VARIATION_LABELS[v] for v in VARIATION_OPTIONS]:
            self.var_dd.set(s["variation"])
            self._on_variation_change(s["variation"])
        if s.get("mode") in ("PAPER", "LIVE"):
            self._mode_var.set(s["mode"])
            self._on_mode_change()
        if s.get("order_type") in ORDER_TYPES:
            self.order_dd.set(s["order_type"])
            self._on_order_type_change(s["order_type"])

        def _set(entry, key):
            if s.get(key) is not None:
                entry.delete(0, "end"); entry.insert(0, str(s[key]))

        _set(self.e_trig,  "trig_offset")
        _set(self.e_lmt,   "lmt_offset")
        _set(self.e_gsl,   "global_sl")
        _set(self.e_mcx_h, "mcx_h")
        _set(self.e_mcx_m, "mcx_m")
        _set(self.e_kc_len,  "kc_len")
        _set(self.e_kc_atr,  "kc_atr")
        _set(self.e_kc_mult, "kc_mult")
        _set(self.e_rsi_len,  "rsi_len")
        _set(self.e_rsi_buy,  "rsi_buy")
        _set(self.e_rsi_sell, "rsi_sell")

        syms = s.get("symbols", {})
        all_enabled = True
        for sym in ALL_SYMBOLS:
            if sym in syms:
                self._sym_vars[sym].set(bool(syms[sym].get("enabled", True)))
                if not syms[sym].get("enabled", True):
                    all_enabled = False
                lot = syms[sym].get("lot", "")
                self._lot_entries[sym].delete(0, "end")
                if lot:
                    self._lot_entries[sym].insert(0, str(lot))
                buf = syms[sym].get("buf", SYM_BUF_DEFAULTS.get(sym, "3.0"))
                self._buf_entries[sym].delete(0, "end")
                self._buf_entries[sym].insert(0, str(buf))
        self._all_var.set(all_enabled)

    def _save_settings(self):
        try:
            SETTINGS_FILE.write_text(
                json.dumps(self._collect_settings(), indent=2), encoding="utf-8")
            self._elog("💾  Settings saved to settings.json")
        except Exception as e:
    ...
    def _on_mode_change(self):
        if self._mode_var.get() == "LIVE":
            self.live_warn.configure(text="⚠️  REAL ORDERS WILL BE PLACED")
        else:
            self.live_warn.configure(text="")
    ...
    def _get_buf_overrides(self) -> dict:
        ...
        for sym, e in self._buf_entries.items():
            ...
            try: result[sym] = float(e.get())
            except: pass
        return result

    def _get_global_sl(self) -> float:
        try: return float(self.e_gsl.get())
        except: return 0.0

    def _get_mcx_end(self) -> tuple:
        try:
            h = int(self.e_mcx_h.get())
            m = int(self.e_mcx_m.get())
            return (h, m)
        except:
            return (23, 30)
    ...
    def _start(self):
        ...
        if not self._client_id or not self._access_token:
            messagebox.showerror("No Credentials",
                "Please go to Token Manager and generate a token first.")
            return
        ...
        if is_live:
            if not messagebox.askyesno("⚠️  LIVE TRADING",
        ...
        self._elog(
            f"Starting — {mode_str}  TF={tf_val}m  {variation}  "
    ...
    def _square_off(self):
        ...
        def _run():
            try:
                ...
                err = str(e); self.after(0, lambda: self._elog(f"❌  {err}"))
        threading.Thread(target=_run, daemon=True).start()
    ...
    def _poll_dashboard(self):
        ...
        try:
            ...
            # Show global SL hit warning
            if snap.get("global_sl_hit"):
                self.status_lbl.configure(
                    text="⛔  GSL HIT", text_color=LIVE_COL)
        except Exception as e:
    ...
    def _render_dashboard(self, snap: dict):
        ...
        mcx_h, mcx_m = snap.get("mcx_session_end", (23, 30))
        ...
        lines.append(
            ...
            f"MCX End {mcx_h:02d}:{mcx_m:02d}  │  {now}  │  WS: {snap['ws_uptime']}"
        )
        lines.append("─" * 178)
        lines.append(
