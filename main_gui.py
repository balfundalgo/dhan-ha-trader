...
Features: Live/Paper, MARKET/SL-M/LIMIT, per-symbol lot+buffer,
          Global SL%, MCX session end, Save/Load settings, shared token
"""
...
class TokenTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.shared_lbl = ctk.CTkLabel(sf, text="Checking…", width=240,
    ...
    def _verify_token(self):
        ...
        def _run():
            try:
                ...
                err = str(e); self.after(0, lambda: self._log(f"❌  {err}"))
        threading.Thread(target=_run, daemon=True).start()
...
class StrategyTab(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        self.tf_dd = ctk.CTkOptionMenu(row1, values=TF_OPTIONS, width=110, height=36,
        ...
        self.var_dd = ctk.CTkOptionMenu(
            ...
            width=310, height=36, fg_color=PANEL_BG, button_color=BORDER,
            button_hover_color=ACCENT, text_color=WHITE_COL,
            font=F_LABEL, dropdown_font=F_LABEL, command=self._on_variation_change)
        self.var_dd.set(VARIATION_LABELS["ha_static"])
        self.var_dd.pack(side="left", padx=6)

        # Row 2: Symbols + lot + buffer
        ...
        ctk.CTkCheckBox(row2, text="All", variable=self._all_var, font=F_LABEL,
                        text_color=WHITE_COL, fg_color=ACCENT, hover_color=ACCENT_H,
        ...
        for sym in ALL_SYMBOLS:
            ...
            ctk.CTkCheckBox(grp, text=sym, variable=var, font=F_LABEL,
                            text_color=WHITE_COL, fg_color=ACCENT, hover_color=ACCENT_H,
            ...
            ctk.CTkLabel(grp, text="buf:", font=("Segoe UI",11),
                         text_color=GREY_COL).pack(side="left")
            buf_e = ctk.CTkEntry(grp, width=52, height=28, fg_color=CARD_BG,
                                 border_color=BORDER, text_color=WHITE_COL, font=F_MONO_S)
        ...
        ctk.CTkRadioButton(row3, text="📄 Paper", variable=self._mode_var, value="PAPER",
                           font=F_LABEL, text_color=WHITE_COL, fg_color=ACCENT,
                           hover_color=ACCENT_H, command=self._on_mode_change
                           ).pack(side="left", padx=6)
        ctk.CTkRadioButton(row3, text="🔴 Live", variable=self._mode_var, value="LIVE",
                           font=F_LABEL, text_color=LIVE_COL, fg_color=LIVE_COL,
                           hover_color="#c0392b", command=self._on_mode_change
                           ).pack(side="left", padx=6)
        self.live_warn = ctk.CTkLabel(row3, text="", font=("Segoe UI",11,"bold"),
                                       text_color=LIVE_COL)
        self.live_warn.pack(side="left", padx=(4, 16))
        ctk.CTkLabel(row3, text="Order:", font=F_LABEL,
                     text_color=WHITE_COL).pack(side="left", padx=(4, 6))
        self.order_dd = ctk.CTkOptionMenu(row3, values=ORDER_TYPES, width=130, height=36,
            fg_color=PANEL_BG, button_color=BORDER, button_hover_color=ACCENT,
            text_color=WHITE_COL, font=F_LABEL, dropdown_font=F_LABEL,
        ...
        self.e_trig = ctk.CTkEntry(self.trig_frame, width=80, height=36,
                                    fg_color=PANEL_BG, border_color=BORDER,
                                    text_color=WHITE_COL, font=F_MONO_S)
        self.e_trig.insert(0, "2.0"); self.e_trig.pack(side="left")
        ...
        ctk.CTkLabel(self.lmt_frame, text="Limit Offset:", font=F_LABEL,
                     text_color=WHITE_COL).pack(side="left", padx=(10,6))
        self.e_lmt = ctk.CTkEntry(self.lmt_frame, width=80, height=36,
                                   fg_color=PANEL_BG, border_color=BORDER,
                                   text_color=WHITE_COL, font=F_MONO_S)
        self.e_lmt.insert(0, "5.0"); self.e_lmt.pack(side="left")
        ctk.CTkLabel(self.lmt_frame, text="pts", font=F_SMALL,
                     text_color=GREY_COL).pack(side="left", padx=(4,0))

        # Row 4: Global SL + MCX session end
        row4 = ctk.CTkFrame(self, fg_color=CARD_BG, corner_radius=10)
        row4.pack(fill="x", padx=14, pady=(0, 4))
        ctk.CTkLabel(row4, text="Global SL:", font=F_LABEL,
                     text_color=WHITE_COL).pack(side="left", padx=(16, 6), pady=10)
        self.e_gsl = ctk.CTkEntry(row4, width=72, height=36, placeholder_text="0",
                                   fg_color=PANEL_BG, border_color=BORDER,
                                   text_color=WHITE_COL, font=F_MONO_S)
        self.e_gsl.pack(side="left", padx=(0, 4))
        ctk.CTkLabel(row4, text="% of balance", font=F_SMALL,
                     text_color=GREY_COL).pack(side="left", padx=(0, 20))
        ctk.CTkLabel(row4, text="MCX Session End (IST):", font=F_LABEL,
                     text_color=WHITE_COL).pack(side="left", padx=(4, 6))
        self.e_mcx_h = ctk.CTkEntry(row4, width=52, height=36, fg_color=PANEL_BG,
                                     border_color=BORDER, text_color=WHITE_COL, font=F_MONO_S)
        self.e_mcx_h.insert(0, "23"); self.e_mcx_h.pack(side="left", padx=(0,2))
        ctk.CTkLabel(row4, text=":", font=F_LABEL, text_color=GREY_COL).pack(side="left")
        self.e_mcx_m = ctk.CTkEntry(row4, width=52, height=36, fg_color=PANEL_BG,
                                     border_color=BORDER, text_color=WHITE_COL, font=F_MONO_S)
        self.e_mcx_m.insert(0, "30"); self.e_mcx_m.pack(side="left", padx=(2,4))
        ctk.CTkLabel(row4, text="(Normal=23:55 | DST=23:30)", font=F_SMALL,
                     text_color=GREY_COL).pack(side="left", padx=(4,0))

        # Row 5: KC/RSI params (conditional)
        ...
        def _param(parent, label, default, width=80):
            ...
            e = ctk.CTkEntry(parent, width=width, height=32, fg_color=PANEL_BG,
                             border_color=BORDER, text_color=WHITE_COL, font=F_MONO_S)
            e.insert(0, str(default)); e.pack(side="left", padx=(0,6), pady=8)
            return e
        ...
        self.start_btn = ctk.CTkButton(btn_row, text="▶  Start", width=130, height=38,
            fg_color=ACCENT, hover_color=ACCENT_H, text_color=WHITE_COL,
            font=F_BTN, command=self._start)
        self.start_btn.pack(side="left", padx=14, pady=10)
        self.stop_btn = ctk.CTkButton(btn_row, text="■  Stop", width=130, height=38,
            fg_color=RED_COL, hover_color=RED_H, text_color=WHITE_COL,
            font=F_BTN, state="disabled", command=self._stop)
        self.stop_btn.pack(side="left", padx=4)
        self.squareoff_btn = ctk.CTkButton(btn_row, text="⬛  Square Off All", width=190, height=38,
            fg_color=ORANGE_COL, hover_color="#b45309", text_color=WHITE_COL,
            font=F_BTN, state="disabled", command=self._square_off)
        self.squareoff_btn.pack(side="left", padx=12)
        ctk.CTkButton(btn_row, text="💾  Save Settings", width=160, height=38,
            fg_color=CARD_BG, hover_color=BORDER, text_color=CYAN_COL,
            font=F_BTN, command=self._save_settings).pack(side="left", padx=8)
        self.info_lbl = ctk.CTkLabel(btn_row, text="", font=F_SMALL, text_color=GREY_COL)
        self.info_lbl.pack(side="left", padx=8)

        # Dashboard
        self.dash = ctk.CTkTextbox(self, font=F_MONO, fg_color=PANEL_BG,
                                   text_color=WHITE_COL, border_color=BORDER,
    ...
    def _collect_settings(self) -> dict:
        return {
            "tf": self.tf_dd.get(), "variation": self.var_dd.get(),
            "mode": self._mode_var.get(), "order_type": self.order_dd.get(),
            "trig_offset": self.e_trig.get(), "lmt_offset": self.e_lmt.get(),
            "global_sl": self.e_gsl.get(), "mcx_h": self.e_mcx_h.get(),
            "mcx_m": self.e_mcx_m.get(),
            "kc_len": self.e_kc_len.get(), "kc_atr": self.e_kc_atr.get(),
            "kc_mult": self.e_kc_mult.get(), "rsi_len": self.e_rsi_len.get(),
            "rsi_buy": self.e_rsi_buy.get(), "rsi_sell": self.e_rsi_sell.get(),
            "symbols": {sym: {"enabled": self._sym_vars[sym].get(),
                              "lot": self._lot_entries[sym].get(),
                              "buf": self._buf_entries[sym].get()}
                        for sym in ALL_SYMBOLS},
        }

    def _apply_settings(self, s: dict):
        if s.get("tf") in TF_OPTIONS:
            self.tf_dd.set(s["tf"])
        ...
        if s.get("order_type") in ORDER_TYPES:
            self.order_dd.set(s["order_type"]); self._on_order_type_change(s["order_type"])
        def _set(e, k):
            if s.get(k) is not None:
                e.delete(0, "end"); e.insert(0, str(s[k]))
        _set(self.e_trig, "trig_offset"); _set(self.e_lmt, "lmt_offset")
        _set(self.e_gsl, "global_sl"); _set(self.e_mcx_h, "mcx_h"); _set(self.e_mcx_m, "mcx_m")
        ...
        for sym in ALL_SYMBOLS:
            if sym in syms:
                en = bool(syms[sym].get("enabled", True))
                self._sym_vars[sym].set(en)
                if not en: all_en = False
                lot = syms[sym].get("lot", "")
                self._lot_entries[sym].delete(0, "end")
    ...
    def _save_settings(self):
        try:
            SETTINGS_FILE.write_text(json.dumps(self._collect_settings(), indent=2), encoding="utf-8")
    ...
    def _on_mode_change(self):
        self.live_warn.configure(
            text="⚠️  REAL ORDERS WILL BE PLACED" if self._mode_var.get() == "LIVE" else "")
    ...
    def _on_variation_change(self, label):
        ...
        show_kc = var in KC_VARIATIONS; show_rsi = var in RSI_VARIATIONS
    ...
    def _set_dash(self, text):
        self.dash.configure(state="normal"); self.dash.delete("1.0", "end")
        self.dash.insert("end", text); self.dash.configure(state="disabled")
    ...
    def _get_kc_params(self):
        try: return {"kc_length": int(self.e_kc_len.get()),
    ...
    def _get_rsi_params(self):
        try: return {"rsi_length": int(self.e_rsi_len.get()),
    ...
    def _start(self):
        ...
        if not self._access_token:
            try:
                ...
                if shared.get("access_token"):
                    ...
                    if shared.get("client_id"): self._client_id = shared["client_id"]
            except Exception: pass
        if not self._client_id or not self._access_token:
            messagebox.showerror("No Credentials", "Please generate a token first.")
            return
        ...
        if is_live and not messagebox.askyesno("⚠️  LIVE TRADING",
        ...
        trig_off   = float(self.e_trig.get() or 0)
        lmt_off    = float(self.e_lmt.get() or 0)
        global_sl  = self._get_global_sl()
        mcx_end    = self._get_mcx_end()
        ...
        self._elog(f"Starting — {mode_str}  TF={tf_val}m  {variation}  "
                   f"{order_type}  GSL={global_sl}%  MCX={mcx_end[0]:02d}:{mcx_end[1]:02d}")
        self.info_lbl.configure(
            text=f"{mode_str}  |  TF={tf_val}m  |  {variation}  |  GSL={global_sl}%")

        def _run():
            try:
                from main import TradingApp
                self._app = TradingApp(
                    strategy_tf=tf_val, symbols_filter=sym_filter,
                    variation=variation, lot_size_overrides=lot_ov,
                    buffer_overrides=buf_ov, live_mode=is_live,
                    order_type=order_type, trigger_offset=trig_off,
                    limit_offset=lmt_off, global_sl_pct=global_sl,
    ...
    def _square_off(self):
        ...
        msg = ("Close ALL LIVE positions with MARKET orders?\n\nAre you sure?"
    ...
    def _render_dashboard(self, snap: dict):
        ...
        gsl   = snap.get("global_sl_pct", 0)
        gsl_r = snap.get("global_sl_rupees", 0)
        mh, mm = snap.get("mcx_session_end", (23, 30))
        gsl_str = f"GSL={gsl}% (₹{gsl_r:.0f})" if gsl > 0 else "GSL=off"
        gsl_hit = snap.get("global_sl_hit", False)
        lines.append(
            f"  {mode}  │  TF {snap['strategy_tf']}m  │  {snap['variation']}  │  "
            f"{ot}  │  {gsl_str}{'  ⛔ STOPPED' if gsl_hit else ''}  │  "
            f"MCX End {mh:02d}:{mm:02d}  │  {now}  │  WS: {snap['ws_uptime']}"
        )
        lines.append("─" * 178)
        lines.append(
            f"  {'Symbol':<14}  {'Contract':<22}  {'LTP':>10}  {'Pos':>6}  "
            f"{'Entry':>10}  {'Pending':>7}  {'Trigger':>10}  "
            f"{'Buf':>6}  {'Lot':>5}  {'uPnL':>12}  {'rPnL':>12}  {'Event':<30}"
        )
        lines.append("─" * 178)
        for s in snap["symbols"]:
            ...
            contr = str(s.get("contract_display") or "-")[:22]
            lines.append(
                f"  {s['name'][:14]:<14}  {contr:<22}  {ltp:>10}  {s['position']:>6}  "
                f"{entry:>10}  {s['pending']:>7}  {trig:>10}  "
                f"{s['buffer']:>6.1f}  {s['lot']:>5}  {upnl:>12}  {rpnl:>12}  "
                f"{str(s['event'])[:30]:<30}"
            )
        lines.append("─" * 178)
        lines.append(
            f"  {'TOTAL':<60}  {'':>6}  {'':>5}  "
            f"{snap['total_unrealized']:>+12.2f}  {snap['total_realized']:>+12.2f}"
        )
        lines.append(f"\n  Packets: ticker={pkt.get(2,0)}  prev_close={pkt.get(6,0)}  "
                     f"other={pkt.get('other',0)}  disconnect={pkt.get(50,0)}")
        if snap["ws_error"]:
            lines.append(f"  ⚠️  WS Error: {snap['ws_error']}")
        lines.append("\n  Last 5 HA Candles")
        lines.append(f"  {'Symbol':<14}  {'Time':>5}  {'Open':>10}  "
