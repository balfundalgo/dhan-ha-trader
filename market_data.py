...
class CandleEngine:
    ...
    def on_tick(self, ltp: float, ltt_epoch: int) -> Optional[Dict[str, Any]]:
        ltp = float(ltp)
        ...
        with self.lock:
            ...
            if self.current is None:
                self.current = {"bucket": bucket, "open": ltp, "high": ltp, "low": ltp, "close": ltp, "tick_count": 1}
                return None
            ...
            if bucket == cur_bucket:
                ...
                self.current["tick_count"] = int(self.current["tick_count"]) + 1
                return None
...
def fetch_intraday_1m_history(access_token: str, inst: Dict[str, Any], days: int = 5, logger=None) -> List[Dict[str, Any]]:
    ...
    payload = {
        ...
        "instrument": str(inst.get("instrument_type", "FUTCOM")),
        "interval": "1",
        "oi": False,
    ...
    try:
        ...
        if not r.ok:
            if logger:
                logger.warning("Backfill failed for %s: HTTP %s %s", inst["name"], r.status_code, r.text[:200])
            return []
        data = r.json()
        ...
        if logger:
            logger.info("Backfill loaded for %s: %d x 1m candles", inst["name"], len(rows))
        return rows
...
class MarketDataEngine:
    def __init__(self, client_id: str, access_token: str, instruments: List[Dict[str, Any]], on_new_1m_candle, on_ltp):
    ...
    def on_message(self, ws, message):
        ...
        if code == RESP_TICKER:
            ...
            finalized = engine.on_tick(ltp, ltt_epoch)
            if finalized:
                self.on_new_1m_candle_cb(sec, finalized)
