...
class CandleEngine:
    ...
    def on_tick(self, ltp: float, ltt_epoch: int) -> None:
        """Update LTP only. Candle formation is handled by RestCandlePoller via REST API."""
        ltp = float(ltp)
...
def fetch_intraday_1m_history(access_token: str, inst: Dict[str, Any], days: int = 5,
    ...
    try:
        ...
        rows = []
        for i in range(n):
            bucket = minute_bucket_epoch(int(ts[i]))
            rows.append({
                "bucket":     bucket,
                "open":       float(op[i]),
                "high":       float(hi[i]),
        ...
        # Drop the current running (incomplete) 1m candle so HA is never
        # seeded from a partially-formed bar. Matches logic from LiveSupertrendScanner.
        if rows:
...
class RestCandlePoller:
    """
    Polls Dhan REST API every POLL_INTERVAL_SEC seconds per instrument.
    Detects newly closed 1m candles by comparing the latest bucket seen
    vs the previous poll, then fires on_new_1m_candle_cb for each new candle.
    WebSocket is used only for LTP (trigger hits) — NOT for candle building.

    Mirrors the pattern from LiveSupertrendScanner.refresh_rest_candles():
      - Fetch last 1 day of 1m data
      - Drop current incomplete candle
      - Compare latest bucket to what we saw last poll
      - Fire callback for every new closed bucket
    """

    POLL_INTERVAL_SEC = 5.0    # poll REST every 5s — new closed candles appear within seconds
    ...
    def __init__(
        ...
        # Track last seen bucket per security_id so we only fire truly new candles
        self._last_seen_bucket: Dict[str, int] = {}

    def _poll_once(self):
        """Fetch latest 1m REST data for all instruments, fire callbacks for new candles."""
        for inst in self.instruments:
            ...
            try:
                ...
                prev_last_bucket = self._last_seen_bucket.get(sec, 0)
                new_candles = [r for r in rows if int(r["bucket"]) > prev_last_bucket]

                if new_candles:
                    # Update engine with all REST rows so seed_from_1m_candle
                    # keeps CandleEngine.current in sync for LTP context
                    ...
                    # Fire callback for each new closed candle
                    for candle in new_candles:
                        if self.logger:
                            self.logger.info(
                                "REST new 1m candle: %s bucket=%s O=%.4f H=%.4f L=%.4f C=%.4f",
                ...
                if self.logger:
                    self.logger.warning("RestCandlePoller error for %s: %s", inst["name"], e)
    ...
    def _run(self):
        while not self.stop_event.is_set():
            ...
            for _ in range(int(self.POLL_INTERVAL_SEC * 10)):
                if self.stop_event.is_set():
                    break
                time.sleep(0.1)
    ...
    def seed_last_buckets(self, rows_by_sec: Dict[str, List[Dict[str, Any]]]):
        """
        Called after startup backfill so the poller knows what buckets
        were already seen — prevents re-firing old candles on first poll.
        """
...
class MarketDataEngine:
    """
    Hybrid engine:
      - WebSocket feed  → LTP updates only (for real-time trigger hits)
      - REST API poll   → accurate 1m OHLC every 15s (for candle strategy)

    This matches the LiveSupertrendScanner pattern exactly.
    """
    ...
    def __init__(
        ...
        # CandleEngine per security — now only used for LTP + prev_close tracking
        ...
        # REST candle poller — does the actual candle detection
    ...
    def seed_last_buckets(self, rows_by_sec: Dict[str, List[Dict[str, Any]]]):
        """
        Pass startup backfill rows to the REST poller so it knows which
        buckets are already processed and won't re-fire them on first poll.
        """
        self.rest_poller.seed_last_buckets(rows_by_sec)

    # ── WebSocket handlers (LTP only) ─────────────────────────────────────────
    def on_open(self, ws):
        with self.lock:
    ...
    def on_message(self, ws, message):
        ...
        if code == RESP_TICKER:
            ...
            # LTP only — no candle building
            engine.on_tick(ltp, ltt_epoch)
    ...
    def start(self):
        # Start WebSocket thread (LTP)
        self.thread = threading.Thread(target=self._run, daemon=True)
        self.thread.start()
        # Start REST poller thread (candles)
