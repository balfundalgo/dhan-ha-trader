...
class HAStaticTriggerStrategy:
    def __init__(
        ...
        # buffer_override from GUI takes priority; falls back to BUFFER_MAP
    ...
    def on_new_1m_candle(self, row_1m: Dict[str, Any]) -> bool:
        ...
        if is_new_session:
            self.session_anchor_epoch = ts

        self.last_1m_bucket = ts
        sb = strategy_bucket_start(ts, self.strategy_tf, session_anchor_epoch=self.session_anchor_epoch)
        ...
        if is_new_session and ts != cur_bucket:
            # Discard the incomplete agg_current (partial candle at session end).
            # DO NOT add it to agg_completed — it is an incomplete candle.
            #
            # two_consecutive: agg_completed already holds yesterday's last COMPLETE
            #   candle. When today's first candle finalises, _rebuild_and_update_pending
            #   will naturally pair it with yesterday's last for HH/LL check. ✅
            #
            # ha_static (and others): clear any pending signal so yesterday's HA
            #   candle does not trigger a carry-over entry on a new session. ✅
            if self.variation != "two_consecutive":
                self._cancel_pending("New session: cross-day pending cleared")
            self.agg_current = {
                "bucket": sb,
                "open": float(row_1m["open"]),
                "high": float(row_1m["high"]),
